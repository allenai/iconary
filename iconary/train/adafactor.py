"""
adafactor from huggingface transformers, copied here since some some versions of this
code do not depend on the right transformer version to have aviable through the library
 """
import math
...
class Adafactor(Optimizer):
  ...
  *warmup_init* options. To use a manual (external) learning rate schedule you should set `scale_parameter=False` and `relative_step=False`.
    ...
    for group in self.param_groups:
      for p in group["params"]:
        if p.grad is None or not p.requires_grad:
          continue
        grad = p.grad.data
        # if "grad_scale" in group:
        #   grad = grad * group["grad_scale"]
          # print(group["grad_scale"])
        ...
        p_data_fp32 = p.data
        # if "grad_scale" in group:
        #   p_data_fp32 = p_data_fp32 / group["grad_scale"]
        ...
        if group["relative_step"] or group["scale_parameter"]:
        ...
        lr = group["lr"]
        if "auto-lr" in group and id(p) in group["auto-lr"]:
          lr *= group["auto-lr"][id(p)]
  ...
  def simulate_step(self, update_internal_state=True, for_params=None):
    out = []
    for group in self.param_groups:
      for p in group["params"]:
        if p.grad is None:
          continue
        if for_params and id(p) not in for_params:
          continue
        grad = p.grad
        require_grad_of_grad = grad.requires_grad
        # if "grad_scale" in group:
        #   grad = grad * group["grad_scale"]
          # print(group["grad_scale"])
        ...
        if factored:
          ...
          if require_grad_of_grad:
            exp_avg_sq_row = exp_avg_sq_row * beta2t + (1.0 - beta2t) * update.mean(dim=-1).detach()
            exp_avg_sq_col = exp_avg_sq_col * beta2t + (1.0 - beta2t) * update.mean(dim=-2).detach()

            if update_internal_state:
              state["exp_avg_sq_col"] = exp_avg_sq_col.detach()
              state["exp_avg_sq_row"] = exp_avg_sq_row.detach()
          ...
          exp_avg_sq = state["exp_avg_sq"]
          if require_grad_of_grad:
            exp_avg_sq = exp_avg_sq.mul(beta2t).add(update, alpha=1.0 - beta2t)
            if update_internal_state:
        ...
        lr = group["lr"]
        if require_grad_of_grad:
          update = update / ((self._rms(update) / group["clip_threshold"]).clamp_(min=1.0))
          update = update * lr
        else:
          update.div_((self._rms(update) / group["clip_threshold"]).clamp_(min=1.0))
          update.mul_(lr)
        ...
        if use_first_moment:
          if require_grad_of_grad or not update_internal_state:
            raise NotImplementedError()
          exp_avg = state["exp_avg"]
          exp_avg.mul_(group["beta1"]).add_(update, alpha=1 - group["beta1"])
          update = exp_avg
